...
public partial class share_cn : System.Web.UI.Page
{
    int pid;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["UserAuthentication"] != null)
        {
            if (!IsPostBack)
            {
                ...
                insertSqlComm = new OleDbCommand("INSERT INTO ba_post(time,p_content,title,status,p_type,login_name,p_img,total) VALUES('" + DateTime.Now.ToString("yyyy/MMM/dd HH:mm:ss") + "','0','0','temporary','0','" + Session["UserAuthentication"].ToString() + "','0','0');SELECT SCOPE_IDENTITY() AS 'identity'", conn);
    ...
    protected void upload_btn_OnClick(object sender, EventArgs e)
    {
        ...
        {
            if (upload_btn.Text.ToString().Equals("變更"))
            {
            ...
            if (extensionFlag && filesizeFlag)
            {
                ...
                upload_status.Text = filename + " 已上載";
                ...
                if (upload_btn.Text.ToString().Equals("上載"))
                {
                    upload_btn.Text = "變更";
            ...
            else
            {
                ...
                if (!extensionFlag)
                {
                    upload_status.Text = "非支援檔案類型";
                }else if(!filesizeFlag){
                    upload_status.Text = "檔案超過 2MB";
                }
            }
        }
        else
    ...
    protected void upload_other_btn_OnClick(object sender, EventArgs e)
    {
        ...
        {
            if (upload_other_btn_temp.ToString().Equals("變更"))
            {
                OleDbConnection chg_conn = new OleDbConnection();
                chg_conn.ConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings["ConnectionString"].ToString();
                chg_conn.Open();
            ...
            if (extensionFlag && filesizeFlag)
            {
                ...
                upload_other_status_temp.Text = filename + " 已上載";
                

                Session[indicator + "_html_img"] += "<A rel='lightbox' HREF='" + img_path + "'><img src='" + img_path + "' style='padding:12px;width:100px;height:auto;'/></A>";


                if (upload_other_btn_temp.Text.ToString().Equals("上載"))
                {
                    upload_other_btn_temp.Text = "變更";
            ...
            else
            {
                ...
                if (!extensionFlag)
                {
                    upload_other_status_temp.Text = "非支援檔案類型";
                }
                else if (!filesizeFlag) {
                    upload_other_status_temp.Text = "檔案超過 2MB";
                }
            }
        }
        else
        {
            upload_other_img_btn.Visible = false;
            upload_other_status_temp.Text = "請選擇檔案";
        }
    }
    ...
    protected void share_btn_OnClick(object sender, EventArgs e)
    {
            ...
            else
            {
                    ...
                    if (chkLoss > 0)
                    {
                        ...
                        Response.Redirect("submit_post_cn.aspx");
                    }
                    else
                    {
    ...
    protected void Page_Error(object sender, EventArgs e)
    {
        ...
        if (ex is HttpRequestValidationException)
        {
            ...
            Server.ClearError();
            Response.AppendHeader("REFRESH", "0.1;URL='http://bearty.hk/share_cn.aspx'");
