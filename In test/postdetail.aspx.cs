...
public partial class postdetail : System.Web.UI.Page
{
            ...
            if ((Request.QueryString["pid"] != null))
            {
                if (!Request.QueryString["pid"].ToString().Equals(""))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        p_content.Text += "</div>";
                        p_content.Text += "<A rel='lightbox' HREF='" + oleReader["p_img"] + "'><img rel='Facebook image' src='" + oleReader["p_img"] + "' width = '100%' id='featured-image'/></A> </br><div style='padding:10px;font-family:Adobe 繁黑體 Std B;font-size:small;-webkit-text-size-adjust:none;'>";
						...
						meta.Content = "http://bearty.hk/postdetail.aspx?pid=" + Request.QueryString["pid"].ToString();
						this.Header.Controls.Add(meta);
						...
						desString.Replace("</br>","");
						desString.Replace("</br>","");
						desString.Replace("<br/>","");
						desString.Replace("<br>","");
						meta.Content = desString + "...";
						this.Header.Controls.Add(meta);
                        ...
                        try
                        {
                            if (!restr.Equals(""))
                            {
                                if (restr.StartsWith("http://bearty.hk") || restr.StartsWith("bearty.hk") || restr.StartsWith("http://www.bearty.hk") || restr.StartsWith("www.bearty.hk"))
                                {
                                    ...
                                    relink.Text += "<a href='postdetail.aspx?pid=" + restr + "' class='event_link'>" + setitle + "</a>";
                                    relb.Visible = true;
                                }
                                else
                                {
    ...
    protected void cm_btn_OnClick(object sender, EventArgs e)
    {
        ...
        Response.Redirect("postdetail.aspx?pid=" + Request.QueryString["pid"] + "#comments");
    }
}
