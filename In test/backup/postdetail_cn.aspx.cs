...
public partial class postdetail_cn : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["UserAuthentication"] != null)
        {
            ...
            username.NavigateUrl = "profile_cn.aspx?user=" + Session["UserAuthentication"].ToString();
                        ...
                        try
                        {
                            if (!restr.Equals(""))
                            {
                                if (restr.StartsWith("http://bearty.hk") || restr.StartsWith("bearty.hk") || restr.StartsWith("http://www.bearty.hk") || restr.StartsWith("www.bearty.hk"))
                                {
                                    ...
                                    relink.Text += "<a href='postdetail_cn.aspx?pid=" + restr + "' class='event_link'>" + setitle + "</a>";
                                    relb.Visible = true;
                                }
                                else
                                {
                                    ...
                                    while (oleReader2.Read())
                                    {
                                        if (!oleReader2["title"].ToString().Equals(""))
                                        {
                                            ...
                                            {
                                                relink.Text += "<a href='postdetail_cn.aspx?pid=" + oleReader2["pid"].ToString() + "' class='event_link'>" + oleReader2["title"].ToString() + "</a></br>";
                                                relb.Visible = true;
                                            }
                                        }
                                    }
    ...
    {
        Response.Redirect("shop_cn.aspx");
    }
